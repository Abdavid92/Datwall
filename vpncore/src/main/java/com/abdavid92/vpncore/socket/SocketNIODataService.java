...
import com.abdavid92.vpncore.IClientPacketWriter;
import com.abdavid92.vpncore.Session;
import com.abdavid92.vpncore.SessionManager;
...
public class SocketNIODataService implements Runnable {
	...
	public void run() {
		...
		selector = SessionManager.getInstance().getSelector();
		runTask();
	}
	/**
	...
	public void setShutdown(boolean shutdown){
		...
		SessionManager.getInstance().getSelector().wakeup();
	}
	...
	private void processSelector(SelectionKey selectionKey, Session session){
		String sessionKey = SessionManager.getInstance().createKey(session.getDestIp(),
