...
class SimManagerNew @Inject constructor(
    @ApplicationContext
    ...
    private suspend fun synchronizeOrCreateSim(subscriptionInfo: SubscriptionInfo, simRepository: Sim?): Sim {
        return if (simRepository != null) {
            simRepository.icon = subscriptionInfo.createIconBitmap(context)
            simRepository.slotIndex = subscriptionInfo.simSlotIndex
            synchronizeSim(simRepository)
        } else {
            val sim = buildSim(subscriptionInfo)
            this.simRepository.create(sim)
            sim
        }
    }

    private suspend fun synchronizeSim(sim: Sim): Sim {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            val defaultData =
    ...
    @RequiresApi(Build.VERSION_CODES.LOLLIPOP_MR1)
    private fun buildSim(subscriptionInfo: SubscriptionInfo): Sim {
        return Sim(
